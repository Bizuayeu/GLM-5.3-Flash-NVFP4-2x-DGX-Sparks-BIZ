...
Use `python -m glm53_setup` from the checkout. Run `python -m unittest discover -s tests -v`, Ruff checks, and `python tools/check_publication.py` after relevant changes. Keep English and Japanese README/setup guides consistent. Original code uses Apache-2.0; preserve all adapted-code notices.
