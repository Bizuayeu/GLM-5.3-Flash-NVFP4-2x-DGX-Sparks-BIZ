...
- **An FA2 profile asks the image for `GLM53_FA2_ATTENTION_API=1` instead of mounting files from the checkout.** Until now `runtime.fa2_attention = true` bind-mounted the FA2 path, its dispatch and the fixed fused unpack over an image that lacked them; the reference image now ships them and `server preflight` refuses an image without the marker.
