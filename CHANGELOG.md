...
- An experimental LPA mode, `split`, in the LPA worker (`glm53_setup/runtime/lpa.py`): every late layer (index above the cut) writes its attention state from the projector output for every token, in prefill and in decode, while its queries and output path keep the layer's own input. The same token then has the same late-layer state however it was computed, which is what a shared prefix cache needs; DeepSeek-V4.1-Flash's causal encoder-decoder builds its decoder KV this way. Merged projections are computed twice and spliced by column (`fused_qkv_a_proj`, the indexer's `wk_weights_proj` and its kpool gate, KDA's `in_proj_bfg_a`). `split-self` feeds the layer's own input through the same path as a control that must reproduce the normal computation exactly. MTP, prefix caching and MLA query skipping are refused with either mode. It is not adopted and stays an experiment: `split-self` reproduced the normal computation bit for bit on the reference pair, the distributed cut-32 projector lost too much under `split`, and a cut-40 projector kept quality but made prefill only 0.8-2.4% faster ([catalog P27](docs/optimization-catalog.md)).
- `server agreement` and `server ask` take `--lpa-mode off|native|split|split-self` on a native-LPA profile: each request is configured through `lpa_configure` first and reset to off afterwards. `native` is the profile's own request; `off` computes normally on the same server. `server agreement --texts` scores a JSON object of texts instead of the built-in ones.
- `lpa-fixture --split` runs capture, off, split-self, split and off on the 4-layer fixture with a synthetic projector: split-self must match off in tokens, log-probabilities (exactly) and KDA state. A cut that does not leave both a KDA and an MLA layer behind it is refused (on the 4-layer fixture, 0 or 1; the runner's default cut 2 leaves only the MLA layer).
