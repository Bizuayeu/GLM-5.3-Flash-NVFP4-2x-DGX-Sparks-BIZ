...
| Repeatability | Identical requests at temperature 0 | same completion nine times of nine, zero log-probability movement; from 1.12.0 every launch computes in the same numerical state (three launches of three) | same, for a request alone (three of three, every launch); from 1.12.0 every launch computes in the same numerical state (three launches of three), the cause of the earlier three states named in [validation](docs/validation.md#full-model-tp2-experimental-scope); with two requests in flight completions differ from a request alone |
