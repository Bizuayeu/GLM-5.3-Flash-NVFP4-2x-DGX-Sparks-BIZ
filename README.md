...
- **Status.** Both profiles are **accepted for routine use since 2026-09-22, for one active sequence** (accepted for the public demonstration at 生成AIなんでも展示会#6 (2026-09-23), and kept for routine use within the same scope), and the published option's two-sequence profile **since 2026-09-23, for two** at up to about 200K tokens each. [SETUP step 6](SETUP.md#6-qualify-the-full-model) records what each acceptance rests on; harness acceptance is recorded per case in [harnesses](docs/harnesses.md#acceptance-matrix-and-status). Other hardware, more sequences than those and video input are outside the accepted scope ([status by scope](#status-by-scope)).
...
| Quality | Teacher-forced NLL: Japanese / English / code / mathematics | 1.5963 / 2.0241 / 0.9479 / 0.5931 | 1.6270 / 1.9946 / 0.9601 / 0.6275 (2026-09-25; 1.6645 / 2.0024 / 1.0031 / 0.6279 on the one-sequence profile, 2026-09-21) |
...
| Repeatability | Identical requests at temperature 0 | Same completion nine times of nine, zero log-probability movement; every launch in the same numerical state from 1.12.0 (three launches of three) | Same, for a request alone; with two requests in flight a completion can differ from the request's lone one, and with `max_num_seqs = 1` requests sent together queue and each repeats its lone completion ([measurements on 1.14.0](docs/benchmarks.md#measurements-on-1140)) |
...
| Full model | Identical requests at temperature 0 | Repeat bit for bit within a launch and, from 1.12.0, across launches: the [repeatability switches](docs/server-configuration.md#repeatability-switches), on in both templates, fix the token order inside each expert, ties in the indexer's top-k, Inductor's per-rank config choice and the sparse-MLA decode split; three launches of three on each profile gave the same completions. A new launch is still checked after every switch (weight digest, decode check, kernel hashes); [how each cause was found](docs/validation.md#repeatability) |
...
| Concurrency | More than one active sequence | **Not supported** in the distributed defaults (`max_num_seqs = 1`; requests queue). The published option's example serves two sequences from 6 GiB per rank and is **accepted for routine use since 2026-09-23** for two at up to about 200K tokens each. A request sharing steps with another can get a different completion (declared behaviour); for completions that repeat under any load serve `max_num_seqs = 1`. More sequences: TP=4 recommended, TP=3 not; neither measured here. [Concurrency scope](docs/validation.md#concurrency-scope) |
