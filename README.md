...
| Full model | Identical requests at temperature 0 | Repeat bit for bit within a launch and, from 1.12.0, across launches: the [repeatability switches](docs/server-configuration.md#repeatability-switches), on in both templates, fix the token order inside each expert, ties in the indexer's top-k and Inductor's per-rank config choice; three launches of three on each profile gave the same completions. A new launch is still checked after every switch (weight digest, decode check, kernel hashes); [how each cause was found](docs/validation.md#repeatability) |
