...
| Repeatability | Identical requests at temperature 0 | same completion nine times of nine, zero log-probability movement; from 1.12.0 every launch computes in the same numerical state (three launches of three); with `max_num_seqs = 2` a request sharing steps with another can differ from its lone completion (2026-09-25) | same, for a request alone (three of three, every launch); from 1.12.0 every launch computes in the same numerical state (three launches of three), the cause of the earlier three states named in [validation](docs/validation.md#full-model-tp2-experimental-scope); with two requests in flight completions differ from a request alone, with `max_num_seqs = 1` requests sent together queue and each repeats its lone completion ([measurements on 1.14.0](docs/benchmarks.md#measurements-on-1140)) |
...
| Full model | Identical requests at temperature 0 | Repeat bit for bit within a launch after two fixes, the token order inside each expert and ties in the indexer's top-k. Until 1.12.0 the pair fell into one of three numerical states across launches (sixteen checked launches of the serving image: eleven, three and two), because each rank timed the configs of the replicated indexer's compiled key norm and one of its three configs sums a row in a different order. `runtime.inductor_deterministic` (on in both templates) picks that config without timing, the same on both ranks: three launches of three on each profile gave the same completions (state 1 on the published option). A new launch is still checked, not assumed: the weight digest, the decode check and the kernel hashes after every switch; [how it was found](docs/validation.md#full-model-tp2-experimental-scope) |
