...
**本リリースには、全モデルTP=2・同時実行1の参照profileで実験結果があります。通常ランチャーと総合的な品質・信頼性は未検収で、ハーネスの受け入れ状態はケース別に[ハーネス](docs/harnesses.ja.md#受け入れ試験一覧と実施状態)に記録しています。この結果だけでデプロイ完了にはしません。**
...
**通過条件:** 対応手順と実際の証跡がそろうまで、本リリースでは保留。
