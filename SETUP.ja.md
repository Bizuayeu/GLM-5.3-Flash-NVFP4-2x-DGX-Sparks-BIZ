...
どちらも一致するダウンロード状態、そのホストでビルド済みのreference image、記入済みの起動設定TOMLが必要です。preflightの検査内容は[起動検査](docs/operations.ja.md#フルモデルの起動検査)にあります。ベースdigestを参照タグへ書き換えないでください。同じ値はイメージ準備とソースパッチの基点でもあります。
