...
**全モデルTP=2・同時実行1の参照profileは、宣言した範囲について、[手順6](#6-フルモデルの検証)に記録した証拠で通常運用として受け入れ済みです（2026-09-22）。その範囲の外（他のハードウェア、配布既定での同時2系列以上、動画入力）は未検収です。公開した任意設定の同時2系列profileは1起動でタスク単位の検査を通しており、通常運用としての受け入れはまだです（[同時実行の範囲](docs/validation.ja.md#同時実行の範囲)）。ハーネスの受け入れ状態はケース別に[ハーネス](docs/harnesses.ja.md#受け入れ試験一覧と実施状態)に記録しています。**
...
**判定:** 2026-09-22より、配信profile（同時1系列）について通常運用としての受け入れが成立。宣言した範囲の外（配布既定での同時2系列以上、動画入力、他のハードウェア）は範囲外のまま。公開した任意設定の同時2系列profileは1起動のタスク単位の証拠があり、起動の積み上がりを待つ（[同時実行の範囲](docs/validation.ja.md#同時実行の範囲)）。
