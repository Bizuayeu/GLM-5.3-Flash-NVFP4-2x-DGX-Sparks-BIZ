...
**全モデルTP=2・同時実行1の参照profileは、宣言した範囲について、[手順6](#6-フルモデルの検証)に記録した証拠で通常運用として受け入れ済みです（2026-09-22）。その範囲の外（他のハードウェア、配布既定での同時2系列以上、動画入力）は未検収です。公開した任意設定の同時2系列profileは、[手順6](#6-フルモデルの検証)と[同時実行の範囲](docs/validation.ja.md#同時実行の範囲)に記録した範囲で2026-09-23から受け入れ済みです。ハーネスの受け入れ状態はケース別に[ハーネス](docs/harnesses.ja.md#受け入れ試験一覧と実施状態)に記録しています。**
...
[実験の適用範囲](docs/validation.ja.md#フルモデルtp2の実験範囲)と[初期ベンチ](docs/benchmarks.ja.md)には、同時実行1での実測結果があります。通常運用としての受け入れは、2026-09-22にその記録済みの証拠で成立しました。別のランチャーの問題ではありません。以下の項目がその根拠で、続く表に各項目の記録先を示します。
...
| 上記の項目 | 記録先 |
|---|---|
| 層のロード・メモリ・余裕・KV・OOMなし | 配信profileについて[ベンチマーク](docs/benchmarks.ja.md)（rankあたりの重み91.34 GiB、FP8 KV各3 GiB、同じ夜の長文benchでのheadの最小空き10.50 GiB）と、起動のたびに走る[起動検査](docs/operations.ja.md#フルモデルの起動検査) |
...
**判定:** 2026-09-22より配布既定の配信profile（同時1系列）について、2026-09-23より公開した任意設定の同時2系列profile（同時2系列・1要求あたり約200K tokenまで）について、通常運用としての受け入れが成立。それらの範囲の外（それを超える同時数、動画入力、他のハードウェア）は範囲外のまま（[同時実行の範囲](docs/validation.ja.md#同時実行の範囲)）。
