...
人とAIの両方が使う、順序付きの作業手順書です。固定バージョンは[ランタイムロック](config/runtime.lock.json)、コマンドの挙動と復旧は[運用文書](docs/operations.md)、試験コマンドと根拠は[検証文書](docs/validation.md)を正典とします。実行前に併読してください。配布構成はテキスト・ツール・画像を受け付け、動画は拒否します。テキスト・ツールを先に検収し、その後に[画像入力](docs/vision.ja.md)を検収します。
