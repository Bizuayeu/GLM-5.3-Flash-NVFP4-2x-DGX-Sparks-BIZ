...
**全モデルTP=2・同時実行1の参照profileは、宣言した範囲について、[手順6](#6-フルモデルの検証)に記録した証拠で通常運用として受け入れ済みです（2026-09-22）。その範囲の外（他のハードウェア、複数系列の同時実行、動画入力）は未検収です。ハーネスの受け入れ状態はケース別に[ハーネス](docs/harnesses.ja.md#受け入れ試験一覧と実施状態)に記録しています。**
...
**判定:** 2026-09-22より、配信profileについて通常運用としての受け入れが成立。宣言した範囲の外（複数系列の同時実行、動画入力、他のハードウェア）は範囲外のまま。
