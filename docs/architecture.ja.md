...
| `tests/` | CPU契約 |
| `tools/` | `check_publication.py`（公開監査）、`release_notes.py`（tagが公開するChangelogの節）、`assess_benchmark.py`、`check_prefix_cache.py`、`decode_check.py` と `decode_divergence.py`（切替の後のdecode検査、[起動契約](launch-safety.ja.md#切替の後のdecode検査)）、`nccl_probe.py`、`prepare_mtp_view.py` |
