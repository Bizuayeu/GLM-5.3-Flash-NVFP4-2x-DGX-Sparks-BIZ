...
本書はTP=2ベンチの方法、MTPなしの基準値、独立施策の全モデル実行（P08・P11・P13〜P15・P17〜P19・P21・P22・checkpoint保持）、リリース候補・200K・256Kの実入力確認と、それを1.3.1で繰り返したsparkDash・200Kの測定を所有します。MTP k=1／k=3の比較は[投機的デコーディング](speculative-decoding.ja.md)、現在の画像入力の既定は[200Kでの画像入力](vision.ja.md)を参照してください。
...
この限定した検証を根拠に、当時の配布既定を**256K・KV各3 GiB**へ更新しました（現在はテキスト専用の代替で、画像入力を含む現在の既定は[200Kでの画像入力](vision.ja.md)）。従来の200Kでの速度・tool-eval・FreedomBenchの点数を新構成で再測定したとは扱いません。一般的な長文品質、すべての履歴編集、多系列、実ハーネス、長時間信頼性の検収は別です。試験のtimeoutとクライアント既定値は別なので、長文の初回入力ではクライアント側にも十分な待ち時間が必要です。
...
prefix cacheをリセットしてから、以前と同じ入力を送りました。[200Kでの画像入力](vision.ja.md)で使った合言葉1つの台帳と、[リリース候補](#200kでの実入力確認)で固定コーパスから組んだ容量いっぱいの要求と3か所参照の要求です。時間はprefillを含む要求全体です。
