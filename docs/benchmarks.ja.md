...
この限定した検証を根拠に、配布既定を**256K・KV各3 GiB**へ更新しました。従来の200Kでの速度・tool-eval・FreedomBenchの点数を新構成で再測定したとは扱いません。一般的な長文品質、すべての履歴編集、多系列、実ハーネス、長時間信頼性の検収は別です。試験のtimeoutとクライアント既定値は別なので、長文の初回入力ではクライアント側にも十分な待ち時間が必要です。
