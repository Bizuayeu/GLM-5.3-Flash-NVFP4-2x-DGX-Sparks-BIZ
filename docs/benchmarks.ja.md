...
**このprofileは20万tokenを超える要求では検収されておらず、261,461 tokenでは失敗します。** 2026-09-20に、この長さのchat要求1本が、prefillの終わり近くで両rankのCUDA illegal memory accessに終わりました。4回試して4回とも同じです。内訳は、このprofileのままで2回、`CUDA_LAUNCH_BLOCKING=1` を足して1回、深さを3に下げて1回です。その後、監視が対を停止しました。同じ要求は、既定の重みでは深さ4でも（231.0 s、正答）深さ3でも完走しました。したがって、失敗は深さではなく、再量子化したcheckpointとそのoverlayに付いてきます。同期報告にすると、エラーはvLLMのslot対応付けのkernelで表面化しました。このkernelは重みに依存しないので、その入力を壊しているものはまだ特定できていません。再量子化した4層fixture（TP=1）は、同じ長さを完走しました。原因を切り分けるまで、このprofileへの要求は、完走実績のある199,652 token以下にしてください。配布している既定は影響を受けません。
