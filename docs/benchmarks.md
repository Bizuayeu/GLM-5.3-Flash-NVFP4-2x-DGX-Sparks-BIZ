...
- The launcher path of 1.15.0 (preflight `cpu_set_available`, the read-back after `docker run`) has not yet run on the pair. The containers were returned to `0-19`: Docker cannot clear a CPU set on a running container, so `HostConfig.CpusetCpus` reads `0-19` until the next switch.
