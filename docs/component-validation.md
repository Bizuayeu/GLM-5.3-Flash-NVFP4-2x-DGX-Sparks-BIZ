...
The retry (`pipeline-v17-pp2-b`) is **not accepted**: startup did not reach a verified result before head management access was lost; its root cause and head's final state remain unconfirmed. The peer was explicitly stopped. Do not infer a host fault's cause from the last model log or count the retry as a numerical pass. Sender/receiver hashes, cache/KDA state, same-image controls and full-model TP/PP performance remain outstanding. PP is not exposed as a normal startup option.
