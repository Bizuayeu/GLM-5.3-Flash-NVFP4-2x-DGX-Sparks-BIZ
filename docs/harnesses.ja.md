...
ハーネスは、モデルと会話し、ファイル操作・ツール実行・履歴・承認を管理するクライアントです。GPU上の推論サーバーとは別の部品です。**公式ZCodeとClaude Codeの両経路を受け入れ試験の必須対象とします。現在はすべてNOT RUNで、連携確認済みではありません。** フルモデルTP=2の起動・基礎API検証が前提です。
...
**現在の状態は全件NOT RUN。API群の後に、共通群HをZCode／Claude Codeそれぞれ別の結果として実施します。** 合格には当該構成の実リクエストと成果物が必要で、文書・ソースの存在や小型fixtureの合格では代用しません。
