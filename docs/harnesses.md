...
These are **planned settings, not a validated deployment recipe**. Do not bypass `server preflight`. Record client version/distribution hash, server source/image/revision and effective arguments first. Use the API port from your server TOML; the example template uses 8893 on loopback.
...
Declare text, tool and image input to match the server (`runtime.vision = true` in the distributed profile; in ZCode, `modalities.input = ["text", "image"]`), and never video, which the server rejects. Do not enable cloud fallback, external integrations or additional agents by default. Point both the main and the helper ("lite") model at the local served ID. Set the model's `limit.context` to the server `max_model_len`, keep `limit.output` at 32000, and raise `modelStream.idleTimeoutMs` above the measured long-context prefill (see [model limits](#zcode-permission-modes-model-limits-and-the-existing-file-guard)). Confirm the selected model and actual destination. The UI locale accepts only the values the client documents (`zcode --help` lists them); an unsupported value invalidates the whole configuration file.
...
**Verified behavior (`20260914-zcode-exists-guard`, Desktop-bundled CLI in headless `--prompt`, local model).** A `Write` to a new file was allowed and the file was created. A `Write` to an existing file became `ask`; headless mode has no permission client, so the tool call was rejected ("No permission client configured") and the file was unchanged. A non-destructive `Bash` command (`ls -la .`) was allowed and answered; `rm doomed.txt` became `ask`, was rejected the same way and the file remained (these two ran after the local API had been restored; an earlier attempt failed on `ECONNRESET` and is kept in the record). In the TUI or Desktop the same `ask` is a prompt; the Desktop GUI was not run.

**Model limits and the compaction budget.** Each custom model entry has `limit.context` and `limit.output`. `limit.context` becomes the client's context window (default 200000 when absent). `limit.output` becomes `maxOutputTokens` and is sent as `max_tokens` on every request (default 32000 when absent; a loopback tap on one headless prompt showed `max_tokens: 32000` on `/v1/chat/completions` with the entry set to 32000); the fixed GLM template always emits a thinking block and vLLM counts it against `max_tokens`, so a small value truncates reasoning plus answer with `finish_reason: length`. Auto-compaction (default strategy `preflight-v1`) uses effective window = `limit.context` − min(`limit.output`, 21000) and starts compacting at effective window − 13000. Two consequences: a leftover small entry compacts far too early (32768/4096 compacts at about 15.7K tokens), and `limit.output` must stay below `max_model_len` − (effective window − 13000), because vLLM rejects a request whose prompt plus `max_tokens` exceeds `max_model_len`. With `limit.context` equal to `max_model_len` that ceiling is about 34000 whether the server allows 204,800 or 262,144 tokens, so keep `limit.output` at 32000 and do not raise it to the model's nominal 64K–128K. Set `limit.context` to the server's `max_model_len` and change both together when the server profile changes. Whether an existing session keeps the model definition it was created with was not verified; start a new session after changing limits.

**Prefix cache and the harness cache display.** Two layers decide what ZCode shows as cache hit. On the server, the LPA-first profile (P22) never publishes an approximated request to the shared cache, and a chat harness turn always leaves a remainder above `lpa.break_even_tokens`, so with the default request mode the shared prefix never grows: a metrics sample on 2026-09-14 showed `vllm:prefix_cache_queries_total` 952,029 against `vllm:prefix_cache_hits_total` 0. The repo's documented escape is per request, `"vllm_xargs": {"glm53_lpa_mode": "off"}`; in ZCode it is set as `provider.<id>.models.<model>.options.extra_body.vllm_xargs.glm53_lpa_mode = "off"` and was observed on the wire as a top-level `vllm_xargs` field. This makes ZCode requests exact; running ZCode through LPA and growing its cache cannot both hold. The distributed template now ships `lpa.enabled = false`, so a plain request already caches (a second identical 19,559-token request restored 13,824 tokens), and the per-request opt-out is kept only so a harness stays exact while a batch run on the same server enables LPA. On the client, ZCode computes the rate from `usage.prompt_tokens_details.cached_tokens`, which vLLM reports only with `--enable-prompt-tokens-details` (`api.prompt_tokens_details = true` in the server TOML); without it the display stays at zero even when the cache hits. Reuse is also block-aligned: this hybrid model pads the attention block to the mamba page size, so the runtime reports `kv lcm block sizes 4608` and only whole 4,608-token blocks can be restored. Exchanges shorter than that never hit. With both layers in place, the same 16,859-token prompt sent twice reported `cached_tokens` 9,216 on the second call (`prefix_cache_hits_total` 0 to 9,216), which is two blocks rather than three because MTP excludes the matched tail block. The first turn of a session is always 0, so less than 100% is expected. `python tools/check_prefix_cache.py --model <served id>` runs this check and names which of the two causes applies when nothing is restored; the guard script and its setup live in [examples/zcode-hooks/](../examples/zcode-hooks/).

**Stream idle timeout.** `modelStream.idleTimeoutMs` (default 600000 in the bundle) counts from the request until the first SSE event, and the local server streams nothing during prefill. On expiry the client aborts, retries with the same prompt, and adds 30 seconds per retry, up to 11 attempts; a long-context turn whose prefill exceeds the value never completes. Measured whole-request latency was 473 seconds at 200K, 506 seconds for a 199,652-token request on the [image profile](vision.md#200k-text-request), and about 606 seconds extrapolated to 256K, so the tested configuration uses 700000 (on 1.5.0 the longest 256K request took 493 seconds with chunk 2048); `0` disables the timer at the cost of stall detection. `network.timeout` is wired to the auxiliary HTTP client; whether it also bounds model streaming was not confirmed.
...
For this fixed GLM template, keep thinking active. It always starts an assistant thinking block and does not read an off switch; avoid `thinking=false` / `enable_thinking=false`. The [model card](https://huggingface.co/zai-org/GLM-5.3-Flash) documents `reasoning_effort=low/high/max`, defaulting to max, and recommends `clear_thinking=true` for chat. Low effort is a chat test profile, not a claim to reproduce max-effort leaderboard scores. In ZCode the session effort is switched with `/effort <level>` (alias `/variant`; `/effort list` shows the current and selectable levels). The selectable set is per model profile: IDs matching `glm-5.3` get `low`, `high`, `max`; `medium` and `xhigh` exist only in the profiles for GPT and Qwen IDs. The level reaches the OpenAI-compatible provider as `reasoning_effort`, and the pinned GLM chat template accepts only `low` and `high`; any other value, including `medium` or an omitted field, is treated as `max`, so a custom level cannot add a middle setting. Operator note: at `max` this template keeps thinking with no practical bound, so a normal coding turn spends most of its `max_tokens` and wall time inside the thinking block; use `high` or lower for everyday work and reserve `max` for a single deliberately hard problem. The matching parser/template leak is tracked in [vLLM #54744](https://github.com/vllm-project/vllm/issues/54744); [PR #54825](https://github.com/vllm-project/vllm/pull/54825) was open and unmerged when reviewed. Do not assume an arbitrary image includes it. The pinned vLLM `385dce36` already contains the XGrammar and speculative-decoding fixes [vLLM #53046](https://github.com/vllm-project/vllm/pull/53046) (`c6e19b3`) and [#52805](https://github.com/vllm-project/vllm/pull/52805) (`12f64b3`); a GitHub compare shows the pinned commit 0 behind both.
...
**Run `20260915-harness-h-sandbox` (2026-09-15).** All eleven shared H cases were executed once through the npm `zcode-app-cli` 3.11.2-24 TUI (`yolo` with the existing-file guard hook, main and lite bound to `glm-5.3-flash-nvidia` at the loopback tunnel, `limit.context` 204800, `limit.output` 32000) against a small fixture repository with a seeded bug and a unittest suite. Under the distribution rule above these results stay recorded per distribution; that distribution is the accepted route, so they are the accepted route's results. The official Desktop GUI and Claude Code have not run any H case. The PARTIAL cells name the untested part.

**Decision (2026-09-22).** The accepted route is the npm `zcode-app-cli` distribution, which is what the operator runs. The official Desktop GUI stays BLOCKED — it cannot be made the required target while [feedback #270](https://github.com/zai-org/feedback/issues/270) stands — and is no longer the required target. Claude Code is skipped by decision, because its configuration conflicts with a subscription setup on the same machine, so CC-01, CC-02 and the remaining API-04 items are closed as not pursued. The PARTIAL npm rows keep their untested sub-items listed, and those sub-items are outside the accepted scope.
...
| API-04 | Anthropic-compatible API | Messages full/SSE, tool_use/tool_result and count_tokens have valid structure, termination and usage | PARTIAL: Messages full response and count_tokens pass at model-default effort; Messages SSE, tool_use/tool_result and error cases NOT RUN; remaining items not pursued (Claude Code route skipped) |
| ZC-01 | ZCode | Custom model is selectable; observed endpoint and model ID match local configuration | BLOCKED ([feedback #270](https://github.com/zai-org/feedback/issues/270)); the GUI itself was never run. Desktop bundled CLI BLOCKED for interactive use: start fails with a missing `@zcode/tui` package ([zai-org/feedback #270](https://github.com/zai-org/feedback/issues/270)); its headless `--prompt` does reach the local served ID (`20260914-zcode-exists-guard`), which is evidence for the runtime, not for the GUI. On the accepted route, npm `zcode-app-cli` 3.11.2-24: provider at the tunnel endpoint, served ID selected, Japanese round trip and a headless prompt answered by the local model |
| ZC-02 | ZCode | Declared input capabilities (text, tools and image; no video) respected; no silent substitution with a default cloud model | Same as ZC-01: official Desktop BLOCKED, GUI never run; on the accepted route, the npm distribution's smoke shows main and lite bound to the local served ID with catalog refresh disabled |
...
For H-08, use the actual server limit from the server TOML; alignment with client context assumptions remains untested. Multi-agent, MCP and image workflows are later, separate tests.
