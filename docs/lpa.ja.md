...
- eager、テキスト専用、TP=2、同時実行1、制御クライアント1つ。sequence-parallel MoE・複数の制御クライアントは未対応。APCなしのMTP k=1・2・3併用は `allow_mtp=true` を明示し、[起動設定TOML](server-configuration.ja.md)では自動設定する（LPAとのk=4・5は拒否する）。k=2のLPAは4層fixtureでだけ確かめており（[部品検証](component-validation.ja.md#apc優先lpaのcache隔離p22)）、LPA workerはイメージに焼き込まれるため、k=2を許した後に作ったイメージが要る。APCは以下のscheduler統合済みP22経路を使う。
