...
併用には、まず計算条件ごとにキャッシュを分離し、近似境界が整合するblockだけを再利用する必要がある。さらに実際のcache hitでKDA checkpoint、残り入力の位置、query省略数、capture／oracleの診断を検証する。hookは既に絶対token位置を使うが、それだけではAPC対応の証明にならない。この契約を実装するまで、起動設定とworkerの両方で併用を拒否する。対応後は、APCで既存prefixを省き、未キャッシュの適用可能な入力をLPAで軽くする構成を実測する。単体の改善率を単純加算しない。
