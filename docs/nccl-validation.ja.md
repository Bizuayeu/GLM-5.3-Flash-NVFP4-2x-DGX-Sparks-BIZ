...
**実モデル。** 同じprofileを、チャネル数かMTUだけを変えて起動しました。すべて同じ再起動の後で、01の監視ダッシュボードは止めています。prefillは新規の38,961 token promptを3回流した中央値です。最小空きメモリは、各rankの監視プロセスの標本から、起動・warmup・計測の全期間で読みました。
...
**MTU。** 9000（RoCEのactive MTUは4096）にすると、prefillは2.3〜2.6%上がり、空きメモリはホストあたり1.1〜1.7 GiB減りました。モデルを動かしていないホストでも同じく1.4 GiBの差があり、NICの受信バッファが大きくなる分（4インタフェース×20キュー×1,024本）と合います。参照pairはMTU 1500で運用します。再起動前に取ったMTU 1500のprefill（446 tok/s）は、MTUではなくホストの状態で低く出ていたので、表から外しています。
