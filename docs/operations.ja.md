...
**直列のフルモデルTP=2配信profileは、宣言した範囲について通常運用として受け入れ済みです（2026-09-22）。** その受け入れと、各項目の証拠の所在は[SETUP手順6](../SETUP.ja.md#6-フルモデルの検証)が記録です。profileが実験段階か通常運用可能かは、記録した受け入れ状態（その記録、READMEの状態表、[ハーネス受け入れ一覧](harnesses.ja.md#受け入れ試験一覧と実施状態)）で示し、コマンド名では示しません。他の機体、配布既定での同時2系列以上、動画入力は受け入れた範囲の外で、公開した任意設定の同時2系列profileは[同時実行の範囲](validation.ja.md#同時実行の範囲)に述べた範囲で2026-09-23から受け入れ済みです。
...
このバージョンから作るimageは、`GLM53_SLOT_MAPPING_GUARD=1` も持ちます。build時に、固定しているvLLMのslot対応付けのkernelへpatchを当て、block tableを行の中だけで読むようにします（vLLMのissue #53982。`glm53_setup/runtime/patch_slot_mapping.py`。ほかのbuild時patchと同じく、sourceのSHA-256を照合します）。これがないと、基準の2台の配信profileは約25万tokenを超える要求で失敗します（[実測](benchmarks.ja.md#基準の2台の配信profile)）。このmarkerを要求する検査はないので、古いimageも起動できます。imageが持っているかは `docker image inspect IMAGE --format '{{json .Config.Env}}'` で確かめられます。固定しているvLLMが上流の修正より先へ進んだら、このpatchは外します。

1.13.0から作るimageは、`GLM53_KPOOL_SEED_STRIDE=1` も持ちます。build時に、固定しているvLLMのkpoolのprefill seed kernelへpatchを当て、indexerの生tailのblockをtail自身のstrideで番地付けするようにします（vLLMのpull request #57477。`glm53_setup/runtime/patch_kpool_seed.py`。sourceのSHA-256を照合します）。これがないと、prefillのたびに要求自身のtail blockがseedされず、最後のtokenのkeyとgateが番号の小さいindexer blockへ書き込まれます。そのblockは別の要求のものであり得ます。壊れるのは長い文脈での疎なtop-kの選択で、prefix cacheから再利用する長いpromptで最も効きます。基準の2台では、patchを入れても測った出力は一つも変わりませんでした（[実測](benchmarks.ja.md#1130での測定)）。このmarkerを要求する検査はありません。固定しているvLLMが上流の修正より先へ進んだら、このpatchは外します。

1.14.0以降でbuildしたimageは `GLM53_MLA_DECODE_CPB_API=1` も持ちます。buildがsparse MLAのSM120 backendにpatchを当て（`glm53_setup/runtime/patch_mla_decode_cpb.py`、`patch-reference` の後のファイルのSHA-256で照合）、`runtime.mla_decode_cpb` がFlashInferの `chunks_per_block` を1系列あたりのtoken数から決められるようにします。上の二つと違い、**profileがkeyを設定するとこのmarkerは必須**で（preflightの行 `mla_decode_cpb_support`）、1.14.0から両方のexampleがkeyを設定します。1.14.0のprofileへ切り替える前にreference imageを作り直すか、keyを `false` にしてください。keyがoffならbackendは固定のとおりFlashInferを呼びます。
