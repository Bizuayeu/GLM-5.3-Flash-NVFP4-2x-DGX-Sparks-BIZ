...
三つのbuild時patchが上流vLLMの不具合を直しており、固定しているvLLMが上流の修正より先へ進んだら外します。どれもほかのbuild時patchと同じくsourceのSHA-256を照合します。検査が要求するのは最後の一つだけです。
...
- `GLM53_MLA_DECODE_CPB_API=1`（1.14.0から作るimage。`glm53_setup/runtime/patch_mla_decode_cpb.py`、`patch-reference` の後のファイルで照合）：sparse MLAのSM120 backendで、`runtime.mla_decode_cpb` がFlashInferの `chunks_per_block` を1系列あたりのtoken数から決められるようにします。profileがkeyを設定すると**必須**で（preflightの行 `mla_decode_cpb_support`）、両方のexampleがkeyを設定します。そのようなprofileへ切り替える前にreference imageを作り直すか、keyを `false` にしてください。
