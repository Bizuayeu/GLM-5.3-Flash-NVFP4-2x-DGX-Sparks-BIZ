...
二つのbuild時patchが上流vLLMの不具合を直しており、固定しているvLLMが上流の修正より先へ進んだら外します。どれもほかのbuild時patchと同じくsourceのSHA-256を照合し、どの検査もそのmarkerを要求しません。
...
`GLM53_MLA_DECODE_CPB_API=1`（1.14.0から作るimage。`glm53_setup/runtime/patch_mla_decode_cpb.py`、`patch-reference` の後のファイルで照合）は、退役した `runtime.mla_decode_cpb` のためのmarkerです。patchしたdecodeの呼び出しにservingは届きません（[起動設定](server-configuration.ja.md#再現性のスイッチ)）。要求するのはkeyを `true` のまま持つprofileだけで（preflightの行 `mla_decode_cpb_support`）、keyを持つprofileには `warnings` に `mla_decode_cpb_retired` が入ります。次のimageのbuildでpatchとmarkerを外すので、それまでにprofileからkeyを外してください。
