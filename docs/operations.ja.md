...
**直列のフルモデルTP=2配信profileは、宣言した範囲について通常運用として受け入れ済みです（2026-09-22）。** その受け入れと、各項目の証拠の所在は[SETUP手順6](../SETUP.ja.md#6-フルモデルの検証)が記録です。profileが実験段階か通常運用可能かは、記録した受け入れ状態（その記録、READMEの状態表、[ハーネス受け入れ一覧](harnesses.ja.md#受け入れ試験一覧と実施状態)）で示し、コマンド名では示しません。他の機体、同時2系列以上、動画入力は受け入れた範囲の外です。
