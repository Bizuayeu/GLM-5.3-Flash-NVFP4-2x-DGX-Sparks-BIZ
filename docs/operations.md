...
**Routine use is accepted within a declared scope**: both profiles for one active sequence (2026-09-22) and the published option's two-sequence profile for two ([concurrency scope](validation.md#concurrency-scope), 2026-09-23). [SETUP step 6](../SETUP.md#6-qualify-the-full-model) is the record of that acceptance and of where each item's evidence lives; the status per scope is shown there, in the README status table and in the [harness acceptance matrix](harnesses.md#acceptance-matrix-and-status), not by a command name. Other hardware, more sequences and video input stay outside it.
...
Two build patches fix upstream vLLM bugs and are removed when the pinned vLLM moves past the upstream fix; each is checked against the source's SHA-256 like the other build patches, and no check requires their markers.
...
`GLM53_MLA_DECODE_CPB_API=1` (images built from 1.14.0; `glm53_setup/runtime/patch_mla_decode_cpb.py`, checked against the file as `patch-reference` leaves it) belongs to the retired `runtime.mla_decode_cpb`, whose patched decode call serving never reaches ([server configuration](server-configuration.md#repeatability-switches)). Only a profile that still sets the key `true` requires it (preflight row `mla_decode_cpb_support`), and a profile that carries the key at all gets a line under `warnings` saying it can be removed. The next image build drops the patch and the marker; remove the key from such profiles before then.
...
**How the cause was found, and what held afterwards.** Stopping Bluetooth on the peer did not stop the churn (new D-Bus connections continued at 7.3 per second, from the display manager's greeter session). Counting new process and thread IDs separated the hosts, 6,642 in 10 seconds on the peer against 176 on the head, which carries more model load; new-process snapshots then showed the login message scripts starting 30 times in 8 seconds, `journalctl -u ssh` listed 218 accepted logins in 60 seconds from the head's fabric address, and their parent was the dashboard. With connection reuse, 30-second counts on the peer fell from about 220 new D-Bus connections to 1, from about 450 KB of `polkitd` growth to none and from about 19,900 new process and thread IDs to 1,191. After a power cycle with Bluetooth enabled again, `polkitd`, `bluetoothd` and `wireplumber` stayed the same size over 300 seconds, and a dashboard restart on 2026-09-17 logged in to the peer 0 times in the next 60 seconds. The cap put on `polkitd` during the incident (`MemoryMax=512M` with `Restart=on-failure`) stays on the peer as a second layer; its restart has not been exercised.
