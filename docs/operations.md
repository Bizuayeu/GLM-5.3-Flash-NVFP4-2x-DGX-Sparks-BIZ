...
Three build patches fix upstream vLLM bugs and are removed when the pinned vLLM moves past the upstream fix; each is checked against the source's SHA-256 like the other build patches. Only the last is required by a check.
...
- `GLM53_MLA_DECODE_CPB_API=1` (images built from 1.14.0; `glm53_setup/runtime/patch_mla_decode_cpb.py`, checked against the file as `patch-reference` leaves it): the sparse-MLA SM120 backend lets `runtime.mla_decode_cpb` take FlashInfer's `chunks_per_block` from the tokens of one sequence. **Required** when the profile sets the key (preflight row `mla_decode_cpb_support`), and both examples set it: rebuild the reference image before switching to such a profile, or set the key to `false`.
