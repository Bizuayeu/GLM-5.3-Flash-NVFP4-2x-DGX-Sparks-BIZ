...
| P15 コンテキスト長sweep | 入力長とKV予算を独立に変える。8K→32K→128K→262,144は検討する測定点であり、事前に容量と対応を確認 | TTFT・decode・memoryが悪化する地点と運用可能範囲を可視化 | **KV各rank1 GiB固定で、1系列の32Kまでの容量・時間を実測。** 測定30要求をpreemptionなしで完了。長文課題品質・併用・128K以上は別。[sweep実測](benchmarks.ja.md#32kまでの独立コンテキスト評価p15) |
...
| P22 APC優先＋未処理部分のLPA | 全状態を揃えて復元したHと適用可能長max(0,N-T-H)で判断。通常計算由来だけを共有し、最初の近似以降は共有登録を抑止 | 既存prefixの再利用と長い残余prefillの短縮、要求間での近似状態の混入防止 | **単独校正、限定品質・運用、MTP／融合／async併用と最終held-outを完了。** prefix再利用重視ではMTPなしを選ぶ。[実装契約](apc-lpa-design.md)／[構成別の結果](benchmarks.ja.md#同一入力を再利用する場合の差) |
