...
| P17 TP=2／PP=2 | PPの中間tensor・mHC post/comb転送を実装・fixture検証後、同じ精度と負荷で比較 | 通信待ち削減の可能性と、stageの直列化・不均衡による損失を測る | **8層PP1対照完了、PP2検証中。** 4層分割はhybrid cacheの要件に不適合。接続復旧後に両失敗runの停止・OOMなしを確認。全モデルA/B未実施。[検証記録](component-validation.md#pipeline-fixture-preparation-and-startup-p17) |
