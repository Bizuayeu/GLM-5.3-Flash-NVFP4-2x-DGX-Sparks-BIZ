...
| P22 APC優先＋未処理部分のLPA | 全状態を揃えて復元したHと適用可能長max(0,N-T-H)で判断。通常計算由来だけを共有し、最初の近似以降は共有登録を抑止 | 既存prefixの再利用と長い残余prefillの短縮、要求間での近似状態の混入防止 | **方針確定、APC単独評価後に実装。** 閾値の実測、登録・要求寿命の契約、fixture、全モデル併用を順に検査。[実装計画](apc-lpa-design.md) |
