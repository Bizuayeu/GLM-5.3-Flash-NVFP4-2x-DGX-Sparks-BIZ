...
| P26 sparse MLA decodeの系列ごとの分け方と、batch不変性の調査 | SM120 sparse MLA decodeでFlashInferの `chunks_per_block` を1系列あたりのtoken数から決め（draftのstepは2、検証のstepは6）、その値でdecodeの入口を呼ぶ。SM120 backendへのsource-pinned patch、profile key `runtime.mla_decode_cpb`（省略＝off）、image marker `GLM53_MLA_DECODE_CPB_API=1`（key使用時は必須）、preflight行。新しい依存なし。あわせて、Marlin MoEを丸ごとのtileに埋める案と、共有stepのdecode行とprefill行を別に計算する案を調べた | 要求のattentionが、decode stepを共有する他の要求に依らなくなる。単独の要求は従来と同じ計算 | **1.14.0で採用、両exampleに設定（2026-09-25）。** 単体：keyなしでは相手がいると要求のattentionの全行が変わり、keyありでは0。参照対：両profileとも単独のcompletion・decode速度・NLL・長文は不変。2系列のcompletionはMoEとprefill共有のstepを通してなお単独と違うので、反復するcompletionは `max_num_seqs = 1` で得る（[1.14.0での測定](benchmarks.ja.md#1140での測定)）。不採用：MoEの埋め（MoEの依存は消えるが、MoEの呼び出しがすべて重くなる）と、stepの分割（共有stepでMoEが1層あたり+0.5〜2.1 ms）。どちらも単独では2系列の反復性を完成させない。上流へflashinfer#5553、vllm#46639として報告 |
