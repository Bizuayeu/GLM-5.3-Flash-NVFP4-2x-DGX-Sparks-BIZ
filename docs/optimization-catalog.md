...
| P15 Context-length sweep | Vary input length and KV budget independently; consider 8K/32K/128K/262,144 only after capacity/support checks | Map prefill/decode/memory breakpoints and usable limits | **Serial capacity/latency through32K measured with fixed KV1 GiB per rank.** All30 measured requests completed without preemption. Long-task quality, combinations and128K+ remain separate. [Measured sweep](benchmarks.md#independent-context-sweep-through-32k-p15) |
...
| P18 MTP + LPA + fusion, optionally Graphs | Compare individual/combined paths on identical assets/tasks with restored controls and observed LPA/speculation/capture activity | Determine interaction, quality/memory regressions and recovery | **Serial MTP3/LPA/fusion/async combination accepted within measured scope.** Paired 2K/8K timing, 24 tasks, tools, 16K capacity and active-LPA disconnect completed. Graphs/batching and enterprise qualification remain separate. [Integration results](benchmarks.md#serial-integration-of-mtp-lpa-fused-unpack-and-async-checks-p18) |
| P19 Prefix caching (APC) | Compare cold/warm repeated system/tools/history and different prefixes; check hybrid state, boundaries and isolation | Lower TTFT and repeated prefill in conversations | **Fixture cache hits and speed benefit observed; full-model adoption pending.** Numerical variation retained; positive-hit isolation needs further checks. Incompatible with current LPA. [Component evidence](component-validation.md#independent-prefix-cache-fixture-p19) |
...
| P22 APC priority with LPA on the uncached suffix | Decide from jointly restored H and eligible work max(0,N-T-H); share only ordinarily computed states and suppress publication from the first approximation onward | Reuse exact prefixes, accelerate long residual prefill, and prevent approximate-state contamination across requests | **Policy agreed; implementation queued after independent APC evaluation.** Threshold calibration, publication/lifecycle contracts, fixtures and full-model combinations are required. [Implementation plan (Japanese)](apc-lpa-design.md) |
