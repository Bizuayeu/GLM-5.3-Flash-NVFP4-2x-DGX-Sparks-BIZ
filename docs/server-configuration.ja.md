...
`runtime.mla_decode_cpb`（任意。未指定はoff＝FlashInferが呼び出しごとに選ぶ固定のまま。exampleはまだ書かない）は、両rankに `GLM53_MLA_DECODE_CPB` を設定し、patchを持つimage（`GLM53_MLA_DECODE_CPB_API=1`）を要求します。FlashInfer 0.6.18のSM120 sparse MLA decodeは2,048の候補を32のchunkに分け、各CTAに `chunks_per_block` 個ずつ受け持たせます。未指定だとstep全体のtoken数からheuristicが選ぶので、2本目の系列が来ると要求の候補の足し方が変わり、呼び出しのたびにFlashInferのAutoTunerをPythonで通ります。keyをonにすると、長さの揃った1〜2系列のdecode stepは、1系列あたりのtoken数で値を決めます：draftの1 tokenは3、深さ3の検証stepの4 tokenは6（rankあたり32 head、top-k 2,048）。それ以外のstepはFlashInferの選択のままです。値はeagerのdecode stepでだけ計ったので、launcherは `runtime.decode_graphs` と一緒のkeyを拒みます。GB10一台のkernel単体では、1系列でも2系列でも固定値はheuristicと同じか速く、要求の行は相手に依らなくなりました。
