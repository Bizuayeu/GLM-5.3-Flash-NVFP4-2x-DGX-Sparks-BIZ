...
| Repeatability | `canonical_moe_order`, `stable_indexer_topk`, `inductor_deterministic` and `mla_decode_cpb` all `true`: identical requests repeat bit for bit and every launch computes in the same numerical state ([repeatability switches](#repeatability-switches)) |
...
Four switches make identical requests repeat bit for bit and every launch compute in the same numerical state; all four are on in both examples. How each source of difference was found and measured is in [validation](validation.md#full-model-tp2-experimental-scope).
...
`runtime.mla_decode_cpb` (both examples from 1.14.0; absent = FlashInfer picks per call as pinned) sets `GLM53_MLA_DECODE_CPB` on both ranks and needs `GLM53_MLA_DECODE_CPB_API=1` (images built from 1.14.0). FlashInfer 0.6.18's SM120 sparse-MLA decode splits the 2,048 candidates into 32 chunks and lets each CTA take `chunks_per_block` of them; a heuristic picks that from the token count of the whole step, so a second sequence changes how a request's candidates are summed. With the key on, a decode step of one or two sequences of equal length takes the value from the tokens of one sequence: 2 for one draft token, 6 for a verification step of four tokens (32 heads per rank, top-k 2,048), the heuristic's own values at one sequence, so a request alone computes as before. The call then goes straight to FlashInfer's decode entry, skipping the wrapper, the custom op and the AutoTuner lookup. Every other step keeps FlashInfer's choice. The values were timed on eager decode steps only, so the launcher refuses the key together with `runtime.decode_graphs`. On the reference pair a request alone gave the same completions, decode speed, NLL and long-input answers with the key as without it, on both profiles ([measurements on 1.14.0](benchmarks.md#measurements-on-1140)).

These switches cover a request alone. With `max_num_seqs` of 2 or more a request that shares steps with another can still get a different completion, through the NVFP4 Marlin MoE (its split along K follows the number of expert blocks in the step) and through the prefill-sized kernels of a step shared with a prefill. For completions that repeat whatever else is in flight, serve `max_num_seqs = 1` ([concurrency scope](validation.md#concurrency-scope)).

### Attention, cache and checkpoint

`runtime.fa2_attention` (false when absent; template `true`) sets `GLM53_FA2_ATTENTION` on both ranks. With `true`, prefill-sized calls of the candidate-preserving NoPE attention (more than six query rows) go through FlashInfer's `BatchMLAPagedAttentionWrapper` with the `fa2` backend and page size one, each row's candidates being its KV pages, instead of the reference computation. The packed `fp8_ds_mla` cache stays as it is; the rows a call touches are unpacked to BF16, because FlashInfer 0.6.18 accepts no FP8 MLA KV off SM90. Every selected candidate is kept, so prefix caching, fused unpack and candidate order are unchanged upstream of the call. Decode steps (at most six rows, MTP depth five) stay on the reference path: `plan()` needs each row's length on the host, one synchronisation per MLA layer, cheap beside a prefill chunk and costly in a decode step. On the reference pair prefill of 38,962 tokens became about 2.2 times faster ([measurements on 1.6.0](benchmarks.md#measurements-on-160)). The fused unpack takes the element count at run time, so a 256K series no longer compiles one Triton kernel per count. The path excludes LPA. The checkout mounts the path, its dispatch and that fused unpack over an image built before them, redundantly on an image that carries `GLM53_FA2_ATTENTION_API=1`.
...
| `GLM53_FA2_ATTENTION_API=1` | Never; without it the checkout mounts the FA2 path over the image | 1.6.0 |
...
Set `mtp.enabled` and `lpa.enabled` independently. MTP selects the view prepared with [prepare_mtp_view.py](../tools/prepare_mtp_view.py) and BF16 Triton drafting; `mtp.num_speculative_tokens` accepts 1 to 5: all five were measured with the requantized checkpoint and 1, 3 and 4 with the pinned one, and both examples use 3 ([speculative decoding](speculative-decoding.md#depth-three-for-both-checkpoints-2026-09-21)). LPA selects `runtime.lpa_image`, mounts the projector read-only and enables the worker extension; it excludes `runtime.fa2_attention`, and combined with MTP it accepts depth 1 or 3 only and requires an image with the explicit MTP-aware LPA worker.
