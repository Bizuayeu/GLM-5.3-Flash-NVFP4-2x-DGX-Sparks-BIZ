...
固定したcheckpointのままの場合、同点を決定的にしたk=4と、それが無いk=3の比較です。数え上げ33.08対32.84、散文19.60対20.68、コード28.37対27.41、短いprompt 28.10対27.38。再量子化したprojectionはすべてのstepを軽くするので、深いdraftの長い検証stepの費用は、その上では相対的に小さくなります。再量子化が無いと、深さ4は文が予測しやすい所で1〜4%得をし、散文では5%損をします。
