...
旧 `[allkv]`・`[llkv]` と対応するイメージキーは `[lpa]`・`runtime.lpa_image` として読み込めます。新旧表記の混在は拒否します。表記だけの変更では起動中のfingerprintを維持し、値を変える場合は再起動が必要です。
...
範囲はTP=2・テキスト／ツール・同時1シーケンス・Marlin W4A16・FP8 KVです。LPAにはeager実行とprefix cache無効が必要です。MTPの下書き数は1と3に限定しています。コンテキスト長・チャンク・キャッシュ量・cut・tailを変えた場合は再測定が必要で、設定検査の合格は品質や必要メモリの保証ではありません。[LPA](llkv-approximation.ja.md) と [MTP](speculative-decoding.ja.md) に検証範囲を記載しています。
