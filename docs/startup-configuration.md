...
`runtime.language_model_only = true` (optional key, true when absent; the template states it) keeps `--language-model-only`, so the vision tower is not loaded and requests stay text/tools only. `false` removes the flag on both ranks and nothing else. Image input is unqualified: the checkpoint's quantization config does not exclude the bf16 vision tower, and the memory cost is unmeasured. Validation fixtures still load text-only regardless of this key.
