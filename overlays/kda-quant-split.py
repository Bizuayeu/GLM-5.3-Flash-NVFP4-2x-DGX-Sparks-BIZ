...
class Glm5NextLinearAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        # kda-quant-overlay: the hack above also left self.quant_config=None -- it is
        ...
        # kda-quant-split-overlay (1.8.0): q, k and v are three column-parallel GEMMs and b, f_a, g_a
        # one merged GEMM, instead of the pinned six-in-one in_proj_qkvbfg_a. Under W4A16 Marlin the
        ...
        # Shards 1 (f_a) and 2 (g_a) are replicated across TP ranks; b is column-parallel.
    ...
    def forward(
        ...
        # kda-quant-split-overlay: three GEMMs for q, k, v, concatenated for the merged short-conv
        # below, and one merged GEMM for b, f_a, g_a.
        qkv = torch.cat(
            [
